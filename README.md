...
hson-live's retained test evidence is publicly accessible for independent verification. The explorer displays frozen evidence generated by `npm run pack` and `npm run certify`, covering all aspects of the system:
...
Test execution occurs through CLI and build tooling using LiveHost. The tests explorer does not discover, run, cancel, or recover tests; it loads the immutable evidence index and retained row artifacts for browsing, viewing, and copying.
