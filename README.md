...
hson-live's retained test evidence is publicly accessible for independent inspection. The explorer displays one immutable direct terminal report selected at static-build time from `/test-evidence/<run-id>`.
...
Categories come from the report, and run, suite, and case results preserve the terminal states `pass`, `fail`, `skip`, `unsupported`, `cancelled`, and `error`. Repository revision and dirty state describe the source that produced the run; they are informational, not admission checks. The explorer progressively loads the index, then only an opened category, suite, or case report and its admitted public attachments.
