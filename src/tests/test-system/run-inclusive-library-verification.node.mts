...
assert.equal(availability.targets.reduce((total, target) => total + target.executableChecks, 0), 692);
...
assert.equal(canonicalCases, 2100);
assert.equal(completedExternalIds.length, 37);
