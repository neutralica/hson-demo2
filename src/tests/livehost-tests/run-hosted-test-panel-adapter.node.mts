...
expect_adapter(livehostResult.reportRev === 15, "178-case LiveHost report reaches batched revision 15");
expect_adapter(livehostSink.updates.flatMap((update) => update.newCases).length === 178, "second hosted mode progressively ingests 178 compact cases");
const livehostFinal = livehostSink.updates.at(-1)?.report.summary;
expect_adapter(livehostFinal?.cases === 178 && livehostFinal.pass === 178 && livehostFinal.fail === 0, "second hosted mode renders the complete passing LiveHost summary");
...
expect_adapter(nodeResult.reportRev === 64, "1081-case aggregate report reaches batched revision 64");
const nodeCases = nodeSink.updates.flatMap((update) => update.newCases);
expect_adapter(nodeCases.length === 1081, "aggregate panel receives exactly 1081 compact cases");
expect_adapter(new Set(nodeCases.map((testCase) => `${testCase.suite}\u0000${testCase.name}`)).size === 1081, "aggregate panel case identities are unique");
...
expect_adapter(nodeFinal?.cases === 1081 && nodeFinal.pass === 1081 && nodeFinal.fail === 0, "aggregate panel renders 1081 passing cases");
