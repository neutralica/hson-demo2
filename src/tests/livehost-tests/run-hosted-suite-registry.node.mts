...
expect_registry(livehost.ok && livehost.summary.suites === 11 && livehost.summary.cases === 178, "registered LiveHost runner passes 178 cases under Node");
expect_registry(nodeAll.ok && nodeAll.summary.suites === 43 && nodeAll.summary.cases === 1081, "registered aggregate runner passes every Node-safe case exactly once");
expect_registry(domCore.ok && domCore.summary.suites === 77 && domCore.summary.cases === 948, "registered DOM runner passes the expanded canonical jsdom and geometry tranche");
...
expect_registry(hostedAll.ok && hostedAll.summary.suites === 126 && hostedAll.summary.cases === 2091 && hostedAll.summary.pass === 2091, "registered complete runner passes every canonical hosted case exactly once");
