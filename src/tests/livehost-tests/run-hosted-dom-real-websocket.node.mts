...
try {
  ...
  expect_dom_ws(result.ok && result.summary.cases === 948 && result.summary.pass === 948, "DOM collection passes 948 canonical cases");
  expect_dom_ws(report?.run.id === result.runId && report.run.suite === "dom/core", "dedicated DOM report host preserves identity");
  expect_dom_ws(first.updates.flatMap((update) => update.newCases).length === 948, "progressive report recovery projects all DOM cases once");
  first.dispose();
