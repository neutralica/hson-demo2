...
expect_boundary(HOSTED_SUITES.length === 126 && HOSTED_SUITES.reduce((total, entry) => total + (entry.cases ?? 0), 0) === 2091, "inventory pins 43 Node-safe, 77 jsdom-hosted, and 6 canvas-hosted suites with 2091 canonical cases");
...
expect_boundary(hostedAll.length === 126 && hostedAllKeys.length === 2091 && new Set(hostedAllKeys).size === 2091, "hosted/all is canonical and non-overlapping");
