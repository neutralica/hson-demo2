...
import { HOSTED_TEST_SUITE_IDS } from "../../app/hosted-test/hosted-test-suite";
...
const unreachable = testSources.filter((path) => !reachable.has(path));
...
console.log(`test surface enumeration: ok (${TEST_SURFACE_CATALOG.length} catalog entries, ${registered.length} hosted suites, ${demoTests.length} demo test scripts, ${testSources.length} reachable test source files)`);
