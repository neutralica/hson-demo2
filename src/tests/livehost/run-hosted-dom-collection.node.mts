...
expect_collection(suites.length === 75 && suites.reduce((total, suite) => total + suite.cases.length, 0) === 933, "canonical list is 75 suites / 933 unique cases");
...
expect_collection(result.ok && result.summary.suites === 75 && result.summary.cases === 933 && result.summary.pass === 933 && result.summary.fail === 0, "direct jsdom run passes every canonical case");
expect_collection(HOSTED_JSDOM_SUITES.length === 75 && count(HOSTED_JSDOM_SUITES) === 933, "inventory matches the executable collection");
...
originalLog(JSON.stringify({ suites: 75, cases: 933, pass: result.summary.pass, initMs, geometryInstallMs, directMs, geometryCleanupMs, cleanupMs }));
