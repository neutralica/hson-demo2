...
try {
  ...
  expect_all(canonical.length === 126 && canonicalKeys.length === 2091, "canonical list contains 126 suites / 2091 cases");
  ...
  expect_all(result.summary.suites === 126 && result.summary.cases === 2091 && result.summary.pass === 2091 && result.summary.fail === 0, "complete remote result passes every canonical case");
  ...
  expect_all(panelCases.length === 2091 && new Set(panelCases.map((testCase) => `${testCase.suite}::${testCase.name}`)).size === 2091, "panel receives every compact case exactly once");
  expect_all(updates.at(-1)?.report.summary.cases === 2091 && updates.at(-1)?.report.summary.pass === 2091, "panel completes from generic recovered state");
  ...
  expect_all(secondResult.runId !== result.runId && secondResult.summary.cases === 2091, "a sequential complete run owns a fresh correlated report");
  expect_all(adapter.capture()?.summary.pass === 2091, "the sequential run starts clean and reconstructs independently");
...
try {
  ...
  expect_all(snapshot.suites === 126 && snapshot.cases === 2091, "the actual hosted/all compact records populate one 126-suite / 2091-case model");
  expect_all(snapshot.metrics.suiteRowsCreated === 126 && snapshot.metrics.caseRowsCreated === 0 && snapshot.metrics.visibleCaseRows === 0, "the completed collapsed projection creates 126 suite rows and zero case rows");
