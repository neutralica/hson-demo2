...
expect_wire(envelopes.length === 47, "real run produces 47 transport envelopes");
expect_wire(envelopes[0]?.prevRev === 1 && envelopes[0].rev === 2, "real envelope range begins 1 to 2");
expect_wire(envelopes.at(-1)?.prevRev === 47 && envelopes.at(-1)?.rev === 48, "real envelope range ends 47 to 48");
validate_hosted_test_report_commit_sequence(envelopes, {
...
expect_wire(replay.rev === 48, "decoded transport replay reaches revision 48");
