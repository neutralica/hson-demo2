...
function reject_upgrade(request: IncomingMessage, socket: import("node:stream").Duplex, status: number, message: string): void {
  const body = `${message}\n`;
  socket.write(
...
export async function start_node_application_host(
  ...
  server.on("upgrade", (request, socket, head) => {
    if (!operational || stopping) {
      reject_upgrade(request, socket, 404, "Host is not accepting WebSocket work.");
      return;
    }
    ...
    if (authorityId === null || authorityId.trim() === "") {
      reject_upgrade(request, socket, 400, "A non-empty livehost authority ID is required.");
      return;
    }
    const application = select_authority(authorityId);
    if (application === undefined) {
      reject_upgrade(request, socket, 404, "No application owns this livehost authority.");
      return;
    }
