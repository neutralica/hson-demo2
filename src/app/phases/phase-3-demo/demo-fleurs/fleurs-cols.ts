...
export function pickCenterColor(
    ...
    if (cultivar === "rosette") {
        ...
        // CHANGED: nudge center away from petal family
         const h = normalizeHue(src.h + 140 + (rng() * 60 - 30));
        const l = clamp(src.l * 0.78, 0.26, 0.62);
        const c = clamp(src.c * 0.68, 0.03, 0.14);
