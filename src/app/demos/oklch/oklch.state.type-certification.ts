...
import {
  create_oklch_store,
  type OKLCH_SCHEMAType,
  type OklchCanonicalState,
  type OklchCanonicalToken,
} from "./oklch.state";
...
type _TokensLocation = Expect<Equal<TokensSnap, OKLCH_SCHEMAType["tokens"]>>;
type _GeneratedTokensFitApplicationDto = Expect<
  OKLCH_SCHEMAType["tokens"] extends readonly OklchCanonicalToken[] ? true : false
>;

const _activePathHandle: LiveMapPathHandle<string> = store.locations.activePath;
const _tokensHandle: LiveMapPathHandle<OKLCH_SCHEMAType["tokens"]> = store.locations.tokens;
