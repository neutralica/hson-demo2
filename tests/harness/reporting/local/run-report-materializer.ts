...
export async function materialize_run_site(runDir:string,report:RunReport,redactor:LocalRedactor):Promise<void>{
 ...
 const categoryRefs=[...cats.values()].map(c=>({id:c.id,file:c.file,status:status(c.suites.map(s=>s.status)),totals:c.suites.reduce((t,s)=>{t.suites++;t.cases+=s.totals.cases;for(const key of terminalStatuses)t[key]+=s.totals[key];return t},{suites:0,cases:0,pass:0,fail:0,skip:0,unsupported:0,cancelled:0,error:0})}));
 for(const c of cats.values())await writeFile(join(site,c.file),json(c));
