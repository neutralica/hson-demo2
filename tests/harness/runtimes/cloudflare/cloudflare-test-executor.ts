import type { TestExecutorDescriptor } from "../../../../src/shared/testing/test-executor-contract";
import type { TestExecutorRegistry } from "../../core/test-executor";
import { make_test_executor_registry } from "../../core/test-executor";
import { all_canonical_portable_test_suites } from "../../hosted/canonical-portable-test-suites";

export const CLOUDFLARE_LOCUS_EXECUTOR = Object.freeze({
  id: "cloudflare-livehost",
  kind: "cloudflare-worker",
  label: "Cloudflare Locus",
  location: "hosted",
  capabilities: Object.freeze({
    provides: Object.freeze(["javascript"] as const),
  }),
  supportsStreaming: true,
  supportsCancellation: true,
}) satisfies TestExecutorDescriptor;

export function make_cloudflare_locus_executor_registry(): TestExecutorRegistry {
  return make_test_executor_registry(CLOUDFLARE_LOCUS_EXECUTOR, all_canonical_portable_test_suites());
}
