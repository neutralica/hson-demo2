...
The browser tests explorer displays frozen evidence generated by `pack` and `certify`. Test discovery, dispatch, cancellation, recovery, and report capture remain internal LiveHost capabilities used by CLI/build tooling; they are not browser explorer actions.
