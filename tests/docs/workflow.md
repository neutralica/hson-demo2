...
The browser Tests explorer consumes one immutable direct terminal report root, `/test-evidence/<run-id>`, chosen at static-build time. Its categories are report-provided and its six terminal states (`pass`, `fail`, `skip`, `unsupported`, `cancelled`, and `error`) remain browsable without certification or deployment-revision admission. It fetches index, category, suite, case, and admitted public attachments progressively. Test discovery, dispatch, cancellation, recovery, and report capture remain CLI/internal capabilities; they are not browser explorer actions, and the browser never reads `.test-reports/current.json`.
...
### Keep the browser tests explorer frozen

The explorer should:

```text
load the immutable index
navigate categories, suites, and cases
load retained row evidence lazily
view and copy retained results
```

It must not discover, dispatch, cancel, recover, or run tests. Execution remains
in CLI/build tooling using LiveHost.

### Keep reporting bounded

Hosted execution may stream progress to internal capture and diagnostic clients,
but that live chronology is not part of the mounted browser explorer.
