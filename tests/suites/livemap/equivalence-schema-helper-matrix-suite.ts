...
export function livemap_equivalence_schema_helper_matrix_suite(): TestSuite {
  return {
    suite: SUITE,
    cases: [
      test("direct-and-attached-schemas-admit-the-same-finite-number", "direct and attached schemas admit the same finite number", () => {
      ...
      test("schema-literals-distinguish-negative-zero", "schema literals distinguish negative zero", () => {
      ...
      test("exact-schemas-treat-dangerous-names-as-own-keys", "exact schemas treat dangerous names as own keys", () => {
      ...
      test("schema-literals-retain-ordered-object-entries", "schema literals retain ordered object entries", () => {
      ...
      test("direct-and-attached-schemas-admit-nested-arrays-and-objects", "direct and attached schemas admit nested arrays and objects", () => {
      ...
      test("direct-and-attached-schemas-admit-null-prototype-records", "direct and attached schemas admit null-prototype records", () => {
      ...
      test("schema-admission-structurally-copies-repeated-references", "schema admission structurally copies repeated references", () => {
      ...
      test("schema-admission-accepts-frozen-objects-and-sealed-dense-arrays", "schema admission accepts frozen objects and sealed dense arrays", () => {
        const value = Object.freeze({ items: Object.seal([1, 2]) }); const schema = hson.liveMap.schema.define((s) => s.unknown);
        return { assertRows: [equal_row("accepted", schema.validateRoot(value).ok, true)] };
      }),
      test("optional-means-missing-while-present-undefined-remains-invalid", "optional means missing while present undefined remains invalid", () => {
      ...
      test("custom-constraints-receive-fresh-detached-values", "custom constraints receive fresh detached values", () => {
        const seen: object[] = []; const schema = hson.liveMap.schema.define((s) => s.unknown.constrain("detached", (value) => { seen.push(value as object); (value as Record<string, JsonValue>).a = 9; return true; }));
      ...
      test("object-keys-expose-canonical-integer-like-order", "object keys expose canonical integer-like order", () => {
        const map = hson.liveMap.fromJson('{"value":{"10":10,"2":2,"1":1}}');
        return { assertRows: [equal_row("keys", map.at(["value"]).object.keys(), ["10", "2", "1"])] };
      }),
      test("object-values-expose-canonical-integer-like-order", "object values expose canonical integer-like order", () => {
      ...
      test("object-entries-expose-dangerous-own-keys", "object entries expose dangerous own keys", () => {
      ...
      test("object-setkey-retains-an-existing-position", "object setKey retains an existing position", () => {
        const map = hson.liveMap.fromJson('{"value":{"10":10,"2":2,"1":1}}'); map.at(["value"]).object.setKey("2", 22);
        return { assertRows: [equal_row("keys", map.at(["value"]).object.keys(), ["10", "2", "1"])] };
      }),
      test("object-rename-retains-the-source-position", "object rename retains the source position", () => {
      ...
      test("object-helper-results-are-detached-and-prototype-safe", "object helper results are detached and prototype-safe", () => {
      ...
      test("array-includes-compares-ordered-nested-objects", "array includes compares ordered nested objects", () => {
      ...
      test("array-indexof-distinguishes-both-zero-signs", "array indexOf distinguishes both zero signs", () => {
      ...
      test("array-unique-uses-ordered-samevalue", "array unique uses ordered SameValue", () => {
      ...
      test("array-removevalue-matches-dangerous-nested-data-exactly", "array removeValue matches dangerous nested data exactly", () => {
