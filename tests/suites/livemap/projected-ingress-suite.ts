...
export function livemap_projected_ingress_suite(): TestSuite {
  ...
  return {
    suite: SUITE,
    cases: [
      result_case(SUITE, "plain-object-admits-and-detaches", "plain object admits and detaches", () => {
        const input = { nested: { value: 1 } };
        const map = hson.liveMap.fromJson(input);
        input.nested.value = 2;
        return { snap: map.snap() };
      }, { snap: { nested: { value: 1 } } }),
      result_case(SUITE, "null-prototype-object-admits-to-an-ordinary-public-object", "null-prototype object admits to an ordinary public object", () => {
      ...
      result_case(SUITE, "dense-array-admits-with-every-own-index", "dense array admits with every own index", () => {
      ...
      result_case(SUITE, "frozen-object-admits-by-copied-value", "frozen object admits by copied value", () => ({
        snap: hson.liveMap.fromJson(Object.freeze({ value: 1 })).snap(),
      }), { snap: { value: 1 } }),
      result_case(SUITE, "sealed-array-admits-by-copied-value", "sealed array admits by copied value", () => ({
        snap: hson.liveMap.fromJson(Object.seal([1, 2])).snap(),
      }), { snap: [1, 2] }),
      result_case(SUITE, "empty-object-remains-an-ordinary-detached-object", "empty object remains an ordinary detached object", () => {
      ...
      result_case(SUITE, "empty-array-remains-a-dense-ordinary-array", "empty array remains a dense ordinary array", () => {
        const snap = hson.liveMap.fromJson([]).snap();
        return { array: Array.isArray(snap), length: (snap as JsonValue[]).length };
      }, { array: true, length: 0 }),
      result_case(SUITE, "isolated-surrogate-code-units-survive-admission", "isolated surrogate code units survive admission", () => {
        const snap = hson.liveMap.fromJson({ value: "\ud800" }).snap(["value"]);
        return { length: (snap as string).length, unit: (snap as string).charCodeAt(0) };
      }, { length: 1, unit: 55296 }),
      result_case(SUITE, "negative-zero-survives-construction-and-public-reads", "negative zero survives construction and public reads", () => ({
        negativeZero: Object.is(hson.liveMap.fromJson({ value: -0 }).snap(["value"]), -0),
      }), { negativeZero: true }),
      result_case(SUITE, "repeated-acyclic-references-become-independent-copies", "repeated acyclic references become independent copies", () => {
      ...
      result_case(SUITE, "own-__proto__-survives-as-public-data", "own __proto__ survives as public data", () => {
      ...
      result_case(SUITE, "own-constructor-survives-as-public-data", "own constructor survives as public data", () => {
      ...
      result_case(SUITE, "own-prototype-survives-as-public-data", "own prototype survives as public data", () => {
      ...
      result_case(SUITE, "nested-mutation-after-admission-cannot-affect-canonical-state", "nested mutation after admission cannot affect canonical state", () => {
      ...
      result_case(SUITE, "separate-public-snapshots-share-no-nested-references", "separate public snapshots share no nested references", () => {
      ...
      result_case(SUITE, "public-dangerous-key-descriptors-are-writable-enumerable-configurable-data", "public dangerous-key descriptors are writable enumerable configurable data", () => {
      ...
      result_case(SUITE, "integer-like-public-enumeration-is-truthful-while-graph-order-remains-canonical", "integer-like public enumeration is truthful while graph order remains canonical", () => {
      ...
      result_case(SUITE, "setmany-admits-an-actual-own-__proto__-key-safely", "setMany admits an actual own __proto__ key safely", () => {
      ...
      result_case(SUITE, "root-replacement-admits-all-three-dangerous-names-safely", "root replacement admits all three dangerous names safely", () => {
      ...
      result_case(SUITE, "admission-preserves-caller-keys-and-descriptors", "admission preserves caller keys and descriptors", () => {
...
export function livemap_projected_ingress_rejection_suite(): TestSuite {
  ...
  const rejection = (caseId: string, name: string, value: () => unknown, reason: string): TestCase => (
    result_case(SUITE, caseId, name, () => rejected_mutation(value()), { reason, rev: 0, feeds: 0, unchanged: true })
  );
  return {
    suite: SUITE,
    cases: [
      result_case(SUITE, "ordinary-getter-rejects-without-invocation-or-mutation", "ordinary getter rejects without invocation or mutation", () => {
        let calls = 0;
        const input = {};
      ...
      result_case(SUITE, "ordinary-setter-rejects-without-invocation-or-mutation", "ordinary setter rejects without invocation or mutation", () => {
        let calls = 0;
        const input = {};
        Object.defineProperty(input, "value", { set(_value) { calls += 1; }, enumerable: true });
        return { ...rejected_mutation(input), calls };
      }, { reason: "ACCESSOR_PROPERTY", rev: 0, feeds: 0, unchanged: true, calls: 0 }),
      rejection("nonenumerable-own-property-rejects", "nonenumerable own property rejects", () => Object.defineProperty({}, "hidden", { value: 1 }), "NONENUMERABLE_PROPERTY"),
      rejection("symbol-keyed-property-rejects", "symbol-keyed property rejects", () => ({ value: 1, [Symbol("extra")]: 2 }), "SYMBOL_KEY"),
      rejection("custom-prototype-rejects", "custom prototype rejects", () => Object.create({ inherited: true }), "UNSUPPORTED_PROTOTYPE"),
      rejection("class-instance-rejects", "class instance rejects", () => new (class Example { value = 1; })(), "UNSUPPORTED_PROTOTYPE"),
      rejection("boxed-primitives-reject", "boxed primitives reject", () => new String("value"), "UNSUPPORTED_PROTOTYPE"),
      rejection("date-rejects", "Date rejects", () => new Date(), "UNSUPPORTED_PROTOTYPE"),
      rejection("map-rejects", "Map rejects", () => new Map(), "UNSUPPORTED_PROTOTYPE"),
      rejection("set-rejects", "Set rejects", () => new Set(), "UNSUPPORTED_PROTOTYPE"),
      rejection("function-rejects", "function rejects", () => () => 1, "UNSUPPORTED_TYPE"),
      rejection("promise-rejects", "Promise rejects", () => Promise.resolve(1), "UNSUPPORTED_PROTOTYPE"),
      rejection("bigint-rejects", "bigint rejects", () => 1n, "UNSUPPORTED_TYPE"),
      rejection("symbol-primitive-rejects", "symbol primitive rejects", () => Symbol("value"), "UNSUPPORTED_TYPE"),
      rejection("nan-rejects", "NaN rejects", () => Number.NaN, "NONFINITE_NUMBER"),
      result_case(SUITE, "both-infinities-reject", "both infinities reject", () => ({ positive: rejected_mutation(Infinity).reason, negative: rejected_mutation(-Infinity).reason }), { positive: "NONFINITE_NUMBER", negative: "NONFINITE_NUMBER" }),
      rejection("sparse-array-rejects", "sparse array rejects", () => new Array(3), "SPARSE_ARRAY"),
      rejection("explicit-undefined-array-item-rejects", "explicit undefined array item rejects", () => [undefined], "UNDEFINED_VALUE"),
      result_case(SUITE, "extra-named-and-numeric-looking-array-properties-reject", "extra named and numeric-looking array properties reject", () => {
      ...
      result_case(SUITE, "array-accessor-index-rejects-without-invocation", "array accessor index rejects without invocation", () => {
        let calls = 0;
        const input = [1];
        Object.defineProperty(input, "0", { get() { calls += 1; return 1; }, enumerable: true });
        return { ...rejected_mutation(input), calls };
      }, { reason: "ACCESSOR_PROPERTY", rev: 0, feeds: 0, unchanged: true, calls: 0 }),
      rejection("subclassed-array-rejects", "subclassed array rejects", () => new (class Values extends Array<number> {})(1, 2), "UNSUPPORTED_PROTOTYPE"),
      result_case(SUITE, "cycles-reject-atomically", "cycles reject atomically", () => {
        const input: Record<string, unknown> = {};
        input.self = input;
        return rejected_mutation(input);
      }, { reason: "CYCLE", rev: 0, feeds: 0, unchanged: true }),
      result_case(SUITE, "throwing-proxy-traps-become-atomic-admission-failures", "throwing proxy traps become atomic admission failures", () => rejected_mutation(new Proxy({}, { getPrototypeOf() { throw new Error("trap"); } })), { reason: "REFLECTION_FAILED", rev: 0, feeds: 0, unchanged: true }),
    ],
  };
}
