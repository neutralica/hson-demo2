...
export function livemap_exact_propagation_suite(): TestSuite {
  return {
    suite: SUITE,
    cases: [
      test("public-feed-enumeration-cannot-rewrite-canonical-integer-key-order", "public feed enumeration cannot rewrite canonical integer-key order", () => {
      ...
      test("public-feed-values-are-fresh-per-listener", "public feed values are fresh per listener", () => {
      ...
      test("public-feed-operation-values-are-fresh-per-listener", "public feed operation values are fresh per listener", () => {
        const map = hson.liveMap.fromJson({ value: { a: 1 } });
        let second: unknown;
      ...
      test("feed-delivery-preserves-negative-zero", "feed delivery preserves negative zero", () => {
      ...
      test("feed-delivery-preserves-dangerous-own-data", "feed delivery preserves dangerous own data", () => {
      ...
      test("repeated-feed-delivery-remains-detached", "repeated feed delivery remains detached", () => {
      ...
      test("direct-links-preserve-exact-integer-key-order", "direct links preserve exact integer-key order", () => {
      ...
      test("mapped-links-preserve-exact-integer-key-order", "mapped links preserve exact integer-key order", () => {
      ...
      test("links-preserve-negative-zero", "links preserve negative zero", () => {
      ...
      test("links-preserve-dangerous-keys-and-isolated-surrogates", "links preserve dangerous keys and isolated surrogates", () => {
      ...
      test("links-preserve-array-splices-exactly", "links preserve array splices exactly", () => {
      ...
      test("links-preserve-objects-inside-arrays", "links preserve objects inside arrays", () => {
      ...
      test("path-handle-links-use-exact-propagation", "path-handle links use exact propagation", () => {
        const source = hson.liveMap.fromJson('{"value":{"10":10,"2":2,"1":1,"drop":0}}');
        const target = hson.liveMap.fromJson({ value: { old: true } });
      ...
      test("listener-mutation-cannot-alter-link-transport", "listener mutation cannot alter link transport", () => {
        const source = hson.liveMap.fromJson('{"value":{"10":10,"2":2,"1":1,"drop":0}}');
        const target = hson.liveMap.fromJson({ value: { old: true } });
      ...
      test("failed-target-propagation-leaves-the-target-atomic", "failed target propagation leaves the target atomic", () => {
        const source = hson.liveMap.fromJson({ value: 0 });
      ...
      test("store-diff-suppresses-unchanged-exact-state", "store diff suppresses unchanged exact state", () => {
        const map = hson.liveMap.fromJson({ value: -0 });
        let calls = 0;
      ...
      test("store-diff-publishes-order-only-changes", "store diff publishes order-only changes", () => {
      ...
      test("store-path-publishes-positive-zero-to-negative-zero", "store path publishes positive zero to negative zero", () => {
        const map = hson.liveMap.fromJson({ value: 0 });
        let observed: unknown;
      ...
      test("store-path-suppresses-negative-zero-to-negative-zero", "store path suppresses negative zero to negative zero", () => {
        const map = hson.liveMap.fromJson({ value: -0 });
        let calls = 0;
      ...
      test("store-listener-mutation-remains-isolated", "store listener mutation remains isolated", () => {
        const map = hson.liveMap.fromJson({ value: { a: 1 } });
        map.sub.path(["value"], (next) => { (next as Record<string, JsonValue>).a = 99; });
        map.replace(["value"], { a: 2 });
        return { assertRows: [equal_row("state", map.snap(["value", "a"]), 2)] };
      }),
      test("selector-equality-remains-object.is-for-primitive-results", "selector equality remains Object.is for primitive results", () => {
      ...
      test("livehost-canonical-commits-carry-exact-transport", "LiveHost canonical commits carry exact transport", async () => {
      ...
      test("livehost-recovery-snapshots-preserve-exact-canonical-order", "LiveHost recovery snapshots preserve exact canonical order", () => {
