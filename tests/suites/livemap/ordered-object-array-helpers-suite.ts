...
export function livemap_ordered_object_array_helpers_suite(): TestSuite {
  return {
    ...
    cases: [
      ...
      test("object-values-expose-canonical-order", "object values expose canonical order", () => {
      ...
      test("object-entries-expose-canonical-order", "object entries expose canonical order", () => {
      ...
      test("object-membership-treats-dangerous-keys-as-own-data", "object membership treats dangerous keys as own data", () => {
      ...
      test("object-getkey-materializes-dangerous-data-safely", "object getKey materializes dangerous data safely", () => {
      ...
      test("replacing-an-existing-key-retains-its-position", "replacing an existing key retains its position", () => {
        const map = hson.liveMap.fromJson('{"value":{"10":10,"2":2,"1":1}}');
      ...
      test("a-new-key-appends", "a new key appends", () => {
      ...
      test("setmany-appends-in-admitted-order", "setMany appends in admitted order", () => {
      ...
      test("rename-retains-the-source-position", "rename retains the source position", () => {
      ...
      test("rename-overwrites-a-destination-at-the-source-position", "rename overwrites a destination at the source position", () => {
        const map = hson.liveMap.fromJson({ value: { a: 1, b: 2, c: 3 } });
      ...
      test("dangerous-keys-can-be-deleted", "dangerous keys can be deleted", () => {
      ...
      test("pick-materializes-the-selected-own-data", "pick materializes the selected own data", () => {
        const map = hson.liveMap.fromJson('{"value":{"10":10,"2":2,"1":1,"tail":0}}');
        const picked = map.at(["value"]).object.pick(["1", "10"]);
        return { assertRows: [own_value_row("ten", picked, "10", 10), own_value_row("one", picked, "1", 1)] };
      }),
      test("omit-retains-canonical-source-order", "omit retains canonical source order", () => {
        const map = hson.liveMap.fromJson({ value: { a: 1, b: 2, c: 3 } });
        return { assertRows: [equal_row("keys", Object.keys(map.at(["value"]).object.omit(["b"])), ["a", "c"])] };
      }),
      test("object-helper-outputs-are-detached", "object helper outputs are detached", () => {
      ...
      test("invalid-object-helper-input-rejects-atomically", "invalid object helper input rejects atomically", () => {
      ...
      test("array-includes-compares-nested-object-order", "array includes compares nested object order", () => {
      ...
      test("array-indexof-compares-nested-object-order", "array indexOf compares nested object order", () => {
      ...
      test("array-helpers-distinguish-positive-and-negative-zero", "array helpers distinguish positive and negative zero", () => {
      ...
      test("array-removevalue-uses-ordered-object-equality", "array removeValue uses ordered object equality", () => {
        const map = hson.liveMap.fromJson({ items: [{ a: 1, b: 2 }, { b: 2, a: 1 }] });
        map.at(["items"]).array.removeValue({ b: 2, a: 1 });
        return { assertRows: [equal_row("remaining", map.at(["items"]).array.toArray(), [{ a: 1, b: 2 }])] };
      }),
      test("array-removeall-preserves-the-other-zero", "array removeAll preserves the other zero", () => {
      ...
      test("array-move-preserves-exact-positions", "array move preserves exact positions", () => {
      ...
      test("array-insertion-preserves-dangerous-nested-data", "array insertion preserves dangerous nested data", () => {
      ...
      test("sparse-array-helper-input-rejects-atomically", "sparse array helper input rejects atomically", () => {
