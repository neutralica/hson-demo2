...
export function livemap_exact_transport_suite(): TestSuite {
  return {
    suite: SUITE,
    cases: [
      test("capture emits exact v1 transport alongside the compatibility view", () => {
      ...
      test("exact restore preserves direct integer-like order", () => {
      ...
      test("exact restore preserves mixed key classes", () => {
      ...
      test("exact restore preserves nested ordered objects", () => {
      ...
      test("exact restore preserves an ordered object inside an array", () => {
      ...
      test("exact restore preserves an array inside an object", () => {
      ...
      test("exact restore preserves dangerous keys as own data", () => {
      ...
      test("exact restore preserves isolated surrogate code units", () => {
        const source = hson.liveMap.fromJson({ text: "\ud800x\udfff" });
        const target = hson.liveMap.fromJson({ old: true });
        target.restore(source.capture());
        return { assertRows: [equal_row("string code units", target.snap(["text"]), "\ud800x\udfff")] };
      }),
      test("exact restore preserves nested negative zero", () => {
      ...
      test("mutating the compatibility view cannot change exact capture bytes", () => {
      ...
      test("restore accepts the minimal exact envelope", () => {
        const source = hson.liveMap.fromJson('{"10":10,"2":2,"1":1}');
        const target = hson.liveMap.fromJson({ old: true });
      ...
      test("exact restore ignores a divergent compatibility value", () => {
        const source = hson.liveMap.fromJson('{"10":10,"2":2,"1":1}');
      ...
      test("exact apply reconstructs complete canonical order", () => {
        const source = hson.liveMap.fromJson('{"10":10,"2":2,"1":1}');
        const capture = source.capture();
        const target = hson.liveMap.fromJson({ old: true });
      ...
      test("exact apply preserves negative zero", () => {
      ...
      test("data commits carry exact replay transport", () => {
      ...
      test("direct commit replay preserves an ordered object inside an array", () => {
        const source = hson.liveMap.fromJson('{"items":[{"10":10,"2":2,"1":1}]}');
      ...
      test("exact replay supports later tail operations", () => {
        const source = hson.liveMap.fromJson('{"items":[{"10":10,"2":2,"1":1}]}');
        const target = hson.liveMap.fromJson('{"items":[{"10":10,"2":2,"1":1}]}');
        target.replay(source.splice(["items"], 1, 0, 4));
      ...
      test("same-order exact replay succeeds", () => {
      ...
      test("exact replay conflicts when previous property order differs", () => {
        const source = hson.liveMap.fromJson('{"target":{"a":1,"b":2}}');
      ...
      test("exact replay conflicts when previous zero differs by sign", () => {
        const source = hson.liveMap.fromJson({ value: 0 });
        const target = hson.liveMap.fromJson({ value: -0 });
      ...
      test("exact replay snapshots input before caller mutation", () => {
        const source = hson.liveMap.fromJson({ value: 0 });
        const target = hson.liveMap.fromJson({ value: 0 });
      ...
      test("exact outputs never silently downgrade to legacy-only shapes", () => {
