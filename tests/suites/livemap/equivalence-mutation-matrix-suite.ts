...
export function livemap_equivalence_mutation_matrix_suite(): TestSuite {
  return {
    suite: SUITE,
    cases: [
      test("construction-matches-transform-for-a-plain-object", "construction matches Transform for a plain object", () => {
      ...
      test("construction-matches-transform-for-a-null-prototype-object", "construction matches Transform for a null-prototype object", () => {
        const value = own_record([["a", 1], ["b", "two"]], null);
        return { assertRows: [equal_row("canonical payload", hson.liveMap.fromJson(value).capture().payload, transform_payload(value))] };
      }),
      test("construction-matches-transform-for-frozen-nested-values", "construction matches Transform for frozen nested values", () => {
        const value = Object.freeze({ nested: Object.freeze({ value: 1 }) });
        return { assertRows: [equal_row("canonical payload", hson.liveMap.fromJson(value).capture().payload, transform_payload(value))] };
      }),
      test("set-closes-positive-zero-to-negative-zero", "set closes positive zero to negative zero", () => {
      ...
      test("set-closes-empty-strings-without-absence-coercion", "set closes empty strings without absence coercion", () => {
      ...
      test("set-closes-isolated-surrogate-code-units", "set closes isolated surrogate code units", () => {
      ...
      test("set-closes-nested-object-and-array-values", "set closes nested object and array values", () => {
        const value = { nested: [{ a: 1 }, [true, null]] }; const map = hson.liveMap.fromJson({ value: null }); map.set(["value"], value);
        return { assertRows: [equal_row("canonical payload", map.capture().payload, transform_payload({ value }))] };
      }),
      test("set-preserves-dangerous-own-data-properties", "set preserves dangerous own data properties", () => {
      ...
      test("setmany-retains-integer-like-positions-and-appends-admitted-order", "setMany retains integer-like positions and appends admitted order", () => {
      ...
      test("setmany-preserves-dangerous-keys-as-own-data", "setMany preserves dangerous keys as own data", () => {
      ...
      test("whole-replacement-adopts-the-complete-admitted-order", "whole replacement adopts the complete admitted order", () => {
        const value = own_record([["z", 1], ["a", 2], ["m", 3]]); const map = hson.liveMap.fromJson({ old: true }); map.replace(value);
        return { assertRows: [equal_row("canonical payload", map.capture().payload, transform_payload(value))] };
      }),
      test("nested-replacement-closes-against-transform", "nested replacement closes against Transform", () => {
      ...
      test("path-update-consumes-one-admitted-result", "path update consumes one admitted result", () => {
      ...
      test("batch-closes-its-staged-candidate-once", "batch closes its staged candidate once", () => {
      ...
      test("object-helper-writes-close-through-the-shared-constructor", "object helper writes close through the shared constructor", () => {
      ...
      test("array-helper-insertion-closes-objects-inside-arrays", "array helper insertion closes objects inside arrays", () => {
      ...
      test("array-helper-replacement-retains-negative-zero", "array helper replacement retains negative zero", () => {
      ...
      test("repeated-references-become-equal-detached-occurrences", "repeated references become equal detached occurrences", () => {
      ...
      test("caller-mutation-after-admission-cannot-alter-a-mutation", "caller mutation after admission cannot alter a mutation", () => {
      ...
      test("repeated-accepted-execution-is-byte-deterministic", "repeated accepted execution is byte deterministic", () => {
