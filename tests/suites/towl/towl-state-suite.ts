...
export function towl_state_suite(): TestSuite {
  ...
  return {
    suite: SUITE,
    cases: [
      towl_case(SUITE, "initial-state-is-canonical-and-json-safe", "initial state is canonical and JSON safe", () => create_towl_state(), {
        phase: "lobby",
      ...
      towl_case(SUITE, "state-factory-returns-independent-frozen-seats", "state factory returns independent frozen seats", () => {
      ...
      towl_case(SUITE, "schema-accepts-the-initial-state", "schema accepts the initial state", () => schema_accepts(create_towl_state()), true),
      towl_case(SUITE, "schema-rejects-unknown-root-keys", "schema rejects unknown root keys", () => schema_accepts({
        ...create_towl_state(),
        extra: true,
      }), false),
      towl_case(SUITE, "schema-rejects-unknown-seat-keys", "schema rejects unknown seat keys", () => schema_accepts({
        ...create_towl_state(),
      ...
      towl_case(SUITE, "schema-accepts-both-exact-win-boundaries", "schema accepts both exact win boundaries", () => ({
      ...
      towl_case(SUITE, "schema-rejects-nonpositive-and-fractional-rounds", "schema rejects nonpositive and fractional rounds", () => ({
      ...
      towl_case(SUITE, "schema-rejects-malformed-seat-fields", "schema rejects malformed seat fields", () => ({
