...
function test_case(suite: string, name: string, body: () => void, input?: string): TestCase {
...
export function transform_quoted_name_acceptance_suite(): TestSuite {
  ...
  const cases = [
    test_case(suite, "established bare names remain canonical", () => {
      expect_equal(compact("<bareName 1>"), "<bareName 1>");
    }, "<bareName 1>"),
    test_case(suite, "single-quoted object member names admit spaces", () => {
      expect_equal(first_property_name("<'white space' 1>"), "white space");
    }, "<'white space' 1>"),
    test_case(suite, "single-quoted element names admit spaces", () => {
    ...
    test_case(suite, "backticks are ordinary quoted-name data", () => {
      expect_equal(first_property_name("<'contains ` a backtick' 1>"), "contains ` a backtick");
    }),
    test_case(suite, "backticks are ordinary string-value data", () => {
      expect_json(JSON.parse(hson.fromHson("<message \"contains ` a backtick\">").toJson().serialize()), {
        message: "contains ` a backtick",
      });
    }),
    test_case(suite, "empty object member names preserve established behavior", () => {
      expect_equal(compact("<'' 1>"), "<'' 1>");
    }),
    test_case(suite, "canonical serialization escapes apostrophes", () => {
      expect_equal(compact("<'don\\'t' true>"), "<'don\\'t' true>");
    }),
    test_case(suite, "canonical serialization escapes backslashes but not backticks", () => {
      expect_equal(compact("<'back\\\\slash ` data' 1>"), "<'back\\\\slash ` data' 1>");
    }),
    test_case(suite, "canonical quoted-name output is byte-deterministic", () => {
    ...
    test_case(suite, "authored parse serialize reparse closes", () => {
    ...
    test_case(suite, "JSON retains decoded demanding property names", () => {
    ...
    test_case(suite, "HTML retains decoded demanding property names", () => {
    ...
    test_case(suite, "ordinary quoted names embed directly in a JavaScript template literal", () => {
      const source = `
<
  'major problem here:' ""
...
`;
    ...
    test_case(suite, "host and HSON escaping layer once for an apostrophe", () => {
...
export function transform_quoted_name_rejection_suite(): TestSuite {
  ...
  const rows: readonly Readonly<{
    ...
    { name: "legacy backtick-delimited object names reject", source: "<`white space` 1>", code: "HSON_NAME_LEGACY_BACKTICK" },
    { name: "legacy backtick-delimited element names reject", source: "<`white space` \"value\"/>", code: "HSON_NAME_LEGACY_BACKTICK" },
    { name: "escaped legacy delimiters do not invoke compatibility syntax", source: "<`tick\\`name` 1>", code: "HSON_NAME_LEGACY_BACKTICK" },
    { name: "raw root backticks reject", source: "`name`", code: "HSON_NAME_LEGACY_BACKTICK" },
    { name: "raw array backticks reject", source: "[1,`name`]", code: "HSON_NAME_LEGACY_BACKTICK" },
    { name: "raw backticks cannot become object values", source: "<name `value`>", code: "HSON_NAME_LEGACY_BACKTICK" },
    { name: "unterminated single-quoted names reject", source: "<'unterminated 1>", code: "HSON_NAME_UNTERMINATED" },
    { name: "raw apostrophes do not double themselves", source: "<'don't' 1>", code: "HSON_NAME_UNTERMINATED" },
    { name: "invalid quoted-name letter escapes reject", source: "<'bad\\qescape' 1>", code: "invalid-name-escape" },
    { name: "escaped backticks are not a quoted-name escape", source: "<'bad\\`escape' 1>", code: "invalid-name-escape" },
    { name: "zero-digit Unicode escapes reject", source: "<'bad\\u' 1>", code: "invalid-name-escape" },
    { name: "two-digit Unicode escapes reject", source: "<'bad\\u12' 1>", code: "invalid-name-escape" },
    { name: "malformed Unicode escapes reject", source: "<'bad\\u12xz' 1>", code: "invalid-name-escape" },
    { name: "trailing quoted-name backslashes reject", source: "<'bad\\", code: "invalid-name-escape" },
    { name: "raw horizontal tabs inside quoted names reject", source: "<'bad\tname' 1>", code: "HSON_NAME_CONTROL_UNESCAPED" },
    { name: "raw line feeds inside quoted names reject", source: "<'bad\nname' 1>", code: "HSON_NAME_CONTROL_UNESCAPED" },
    { name: "raw carriage returns inside quoted names reject", source: "<'bad\rname' 1>", code: "HSON_NAME_CONTROL_UNESCAPED" },
    { name: "single quotes do not delimit root string values", source: "'value'", code: "HSON_QUOTE_KIND_UNSUPPORTED" },
    { name: "single quotes do not delimit object string values", source: "<'name' 'value'>", code: "HSON_QUOTE_KIND_UNSUPPORTED" },
    { name: "single quotes do not delimit attribute values", source: "<name attr='value'/>", code: "HSON_QUOTE_KIND_UNSUPPORTED" },
    { name: "double quotes do not delimit object member names", source: "<\"name\" 1>", code: "HSON_OBJECT_EXTRA_VALUE" },
    { name: "quoted object members require owned values", source: "<'name'>", code: "missing-object-member-value" },
    { name: "quoted sibling ownership requires trivia", source: "<'left' 1'right' 2>", code: "HSON_REQUIRED_TRIVIA_MISSING" },
    { name: "legacy rejection owns exact source location", source: "\n  <`legacy` 1>", code: "HSON_NAME_LEGACY_BACKTICK", location: { index: 4, line: 2, column: 4 } },
    { name: "escape rejection owns exact backslash location", source: "\n  <'bad\\q' 1>", code: "invalid-name-escape", location: { index: 8, line: 2, column: 8 } },
  ]);
