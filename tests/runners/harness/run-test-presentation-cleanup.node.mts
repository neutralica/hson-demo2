...
const plan: TestRunPlan = Object.freeze({
  ...
  executorId: "node",
  selectionIds: Object.freeze(["transform/canonical::a", "transform/canonical::b", "transform/opaque", "browser/demo::journey"]),
  suites: Object.freeze([
    ...
    Object.freeze({ id: "browser/demo", title: "Browser", subject: "livehost", collections: Object.freeze([]), provenance: "hson-demo2", order: 2, executionShape: "browser-journeys", cases: Object.freeze([
...
Object.assign(report.suiteRuns[2]!.counts, { total: 1, executed: 1, passed: 1 });
const summary = hosted_test_projection_summary(report);
const footer = hosted_test_projection_footer(summary, null);
check(summary.tests.total === 8, "presentable tests combine canonical cases, opaque checks, and browser journeys");
check(summary.tests.passed === 5, "presentable passed tests combine canonical cases, opaque checks, and browser journeys truthfully");
check(summary.tests.failed === 3, "failed tests combine underlying failure counts without changing them");
check(summary.canonical.total === 2 && summary.launchers.observedChecks === 5 && summary.browser.total === 1, "underlying case, check, and browser counts remain distinct");
check(footer.map((entry) => entry.label).join("|") === "suites|tests|passed|failed|elapsed", "normal summary is five sparse values with passed and no suite-failed KPI");
check(footer.at(-1)?.value === "—", "queued summary makes no elapsed claim");

const cleanChronology = make_hosted_test_chronology();
cleanChronology.begin();
const queuedLines = cleanChronology.ingest(report);
check(queuedLines.join("|") === "queued", "clean Logger emits one queued line");
...
check(cleanChronology.ingest(runningReport).length === 0 && hosted_test_running_readout(runningReport) === "running · 1/3 · transform/canonical", "clean Logger projects one replaceable running context outside accumulated chronology");
...
check(hosted_test_running_readout(advancedRunningReport) === "running · 2/3 · transform/opaque", "live running context advances beyond the first suite without accumulated rows");
check(JSON.stringify(advancedRunningReport) === advancedAuthorityBefore, "live running projection creates no synthetic report mutation");
...
check(passedLines.join("|") === "passed · 25.0 ms", "clean Logger emits only final passed elapsed, without successful count spam");
...
check(failureLines.includes("fail transform/canonical") && failureLines.includes("infrastructure transform/canonical — worker vanished"), "failure and infrastructure chronology remain visible");
check(failureLines.includes("stdout transform/canonical — meaningful failure output"), "exceptional failed stdout remains visible");
...
check(cancelledLines.includes("cancelled transform/canonical") && cancelledLines.includes("cancelled · 35.0 ms"), "suite and terminal cancellation remain visible in sparse chronology");
...
check(rendered.at(-1) === null && !stopwatch.active(), "queued stopwatch renders no elapsed claim and owns no timer");
const running = { ...timerReport.run, status: "running" as const, startedAt: 900 };
stopwatch.update(running);
check(rendered.at(-1) === 100 && stopwatch.active(), "running stopwatch renders elapsed locally and schedules ticking");
now = 1_250;
scheduled?.();
check(rendered.at(-1) === 350, "running elapsed advances without report traffic");
stopwatch.update({ ...running, status: "passed", completedAt: 1_200, timing: { runnerMs: 300, hostMs: 310 } });
check(rendered.at(-1) === 300 && !stopwatch.active() && cancelledTimers === 1, "terminal stopwatch freezes authoritative duration and cancels its timer");
check(JSON.stringify(timerReport) === authorityBefore, "visual ticking does not mutate report state");
stopwatch.dispose();
check(!stopwatch.active(), "stopwatch disposal leaves no timer");
...
check(circuitCalls[0]?.capture === false && captures.size === 1, "authoritative Transform execution does not capture the detailed artifact set");
const caseKey = "transform/inspection-proof::whitespace.exact" as CaseKey;
const reproduced = await captures.get(caseKey)!();
check(circuitCalls[1]?.capture === true && circuitCalls[1]?.entry === circuitCalls[0]?.entry && circuitCalls[1]?.times === circuitCalls[0]?.times, "eligible case reproduction uses the actual tested route with capture enabled");
check((reproduced.artifacts ?? []).map((artifact) => artifact.label).join("|") === "authored input|canonical JSON|projected HTML", "reproduction preserves actual artifact phase order");
check((reproduced.artifacts ?? [])[0]?.node !== undefined, "reproduction retains corresponding HsonNode evidence");
check((originalResult as { metaPatch?: unknown }).metaPatch !== undefined, "inspection reproduction remains separate from the authoritative case result");
...
check(copied.indexOf("authored input") < copied.indexOf("canonical JSON") && copied.indexOf("canonical JSON") < copied.indexOf("projected HTML"), "Copy exposes the same complete ordered circuit");
check(viewed.indexOf("authored input") < viewed.indexOf("canonical JSON") && viewed.indexOf("canonical JSON") < viewed.indexOf("projected HTML"), "View exposes the same complete ordered circuit");
...
check(JSON.stringify(originalResult) === originalResultBeforeFailedInspection, "reproduction failure surfaces separately without changing the authoritative result");

console.log(JSON.stringify({ suite: "test-presentation-cleanup", checks, reproducedArtifacts: diagnostic.artifacts.length }));
